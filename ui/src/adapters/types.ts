...
export type { TranscriptEntry, StdoutLineParser, CreateConfigValues } from "@penclipai/adapter-utils";

export interface StatefulStdoutParser {
  parseLine: (line: string, ts: string) => import("@penclipai/adapter-utils").TranscriptEntry[];
  reset: () => void;
}
...
export interface TranscriptParserSource {
  parseStdoutLine: (line: string, ts: string) => import("@penclipai/adapter-utils").TranscriptEntry[];
