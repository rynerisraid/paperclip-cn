...
import type { Issue, Project, WorkspaceRuntimeService } from "@penclipai/shared";
...
export function IssueProperties({
  ...
  const blockedByTrigger = blockedByIds.length > 0 ? (
    ...
    <span className="text-sm text-muted-foreground">{t("No blockers", { defaultValue: "No blockers" })}</span>
  );
  ...
  return (
    <div className="space-y-4">
      <div className="space-y-1">
        ...
        <PropertyPicker
          inline={inline}
          label={t("Blocked by", { defaultValue: "Blocked by" })}
          open={blockedByOpen}
          onOpenChange={(open) => {
