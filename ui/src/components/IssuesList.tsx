...
const quickFilterPresets = [
  { labelKey: "All", statuses: [] as string[] },
  { labelKey: "Active", statuses: ["todo", "in_progress", "in_review", "blocked"] },
  { labelKey: "Backlog", statuses: ["backlog"] },
  { labelKey: "Done", statuses: ["done", "cancelled"] },
];
...
export function IssuesList({
  ...
  const normalizedIssueSearch = (issueSearch.trim().length > 0 ? issueSearch : deferredIssueSearch).trim().toLowerCase();
  ...
  const groupedContent = useMemo(() => {
    ...
    if (viewState.groupBy === "status") {
      ...
      return statusOrder
        ...
        .map((s) => ({ key: s, label: translateStatusLabel(t, s), items: groups[s]! }));
    }
    if (viewState.groupBy === "priority") {
      const groups = groupBy(filtered, (i) => i.priority);
      return priorityOrder
        .filter((p) => groups[p]?.length)
        .map((p) => ({ key: p, label: translatePriorityLabel(t, p), items: groups[p]! }));
    }
    // assignee
      ...
      label:
        key === "__unassigned"
          ...
          : key.startsWith("__user:")
            ? (formatAssigneeUserLabel(key.slice("__user:".length), currentUserId) ?? t("User", { defaultValue: "User" }))
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between gap-2 sm:gap-3">
        <div className="flex min-w-0 items-center gap-2 sm:gap-3">
          <Button size="sm" variant="outline" onClick={() => openNewIssue(newIssueDefaults())}>
            ...
            <span className="hidden sm:inline">{t("issuesList.newIssue", { defaultValue: "New Issue" })}</span>
          </Button>
          ...
          <Popover>
            <PopoverTrigger asChild>
              <Button variant="ghost" size="sm" className={`text-xs ${activeFilterCount > 0 ? "text-blue-600 dark:text-blue-400" : ""}`}>
                ...
                <span className="hidden sm:inline">{activeFilterCount > 0 ? `${t("issuesList.filters", { defaultValue: "Filters" })}: ${activeFilterCount}` : t("issuesList.filter", { defaultValue: "Filter" })}</span>
                {activeFilterCount > 0 && (
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {quickFilterPresets.map((preset) => {
                      ...
                      return (
                        ...
                        >
                          {t(preset.labelKey, { defaultValue: preset.labelKey })}
                        </button>
                      );
                    })}
                  </div>
                    ...
                    <div className="space-y-0.5">
                      {statusOrder.map((s) => (
                        <label key={s} className="flex items-center gap-2 px-2 py-1 rounded-sm hover:bg-accent/50 cursor-pointer">
                          ...
                          />
                          <StatusIcon status={s} />
                          <span className="text-sm">{translateStatusLabel(t, s)}</span>
                        </label>
                      ))}
                    </div>
                  </div>
                      ...
                      <div className="space-y-0.5">
                        {priorityOrder.map((p) => (
                          <label key={p} className="flex items-center gap-2 px-2 py-1 rounded-sm hover:bg-accent/50 cursor-pointer">
                            ...
                            <span className="text-sm">{translatePriorityLabel(t, p)}</span>
                          </label>
                        ))}
                      </div>
                    </div>
      ...
      {!isLoading && filtered.length === 0 && viewState.viewMode === "list" && (
        <EmptyState
          icon={CircleDot}
          message={t("No issues match the current filters or search.", { defaultValue: "No issues match the current filters or search." })}
          action={t("Create Issue", { defaultValue: "Create Issue" })}
                                ...
                                >
                                  {issue.assigneeAgentId && agentName(issue.assigneeAgentId) ? (
                                    ...
                                    <span className="inline-flex items-center gap-1.5 text-xs">
                                      ...
                                      {formatAssigneeUserLabel(issue.assigneeUserId, currentUserId) ?? t("User", { defaultValue: "User" })}
                                    </span>
                                  ) : (
