  ...
  if (routines.length === 0) {
    return (
      <div className="rounded-lg border border-border px-3 py-8 text-center text-sm text-muted-foreground">
        {emptyMessage ?? t("No managed routines.", { defaultValue: "No managed routines." })}
      </div>
    );
  }
        ...
        return (
          <div key={routine.key} className="last:[&_a]:border-b-0">
            <RoutineListRow
              ...
              href={href}
              configureLabel={t("Configure", { defaultValue: "Configure" })}
              managedByLabel={managedBy
                ? t("Managed by {{name}}", { defaultValue: "Managed by {{name}}", name: managedBy })
                : null}
              runNowButton
              hideArchiveAction
                  ...
                  {routine.cronExpression ? (
                    <span>
                      {t("Schedule {{expression}}", {
                        ...
                        expression: routine.cronExpression,
                      })}
                    </span>
                  ) : null}
                </span>
            ...
            {hasRepairActions ? (
              ...
              >
                <span>
                  {missingRefs.length
                    ? t("Missing {{refs}}", {
                        defaultValue: "Missing {{refs}}",
                        refs: missingRefs.map((ref) => `${ref.resourceKind}:${ref.resourceKey}`).join(", "),
                      })
                    : t("Routine defaults can be repaired.", {
                        defaultValue: "Routine defaults can be repaired.",
                      })}
                </span>
                  ...
                  {onReconcile ? (
                    ...
                    >
                      {reconcilingRoutineKey === routine.key
                        ? t("Reconciling...", { defaultValue: "Reconciling..." })
                        : t("Reconcile", { defaultValue: "Reconcile" })}
                    </Button>
                  ) : null}
                  {onReset ? (
                    <Button
