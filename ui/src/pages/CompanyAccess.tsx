...
import { HUMAN_COMPANY_MEMBERSHIP_ROLE_LABELS, PERMISSION_KEYS, type PermissionKey } from "@penclipai/shared";
...
export function CompanyAccess() {
  ...
  const updateMemberMutation = useMutation({
    ...
    onSuccess: async () => {
      ...
      pushToast({
        title: t("Member updated", { defaultValue: "Member updated" }),
        tone: "success",
      });
    },
    onError: (error) => {
      pushToast({
        title: t("Failed to update member", { defaultValue: "Failed to update member" }),
  ...
  const approveJoinRequestMutation = useMutation({
    ...
    onSuccess: async () => {
      await refreshAccessData();
      pushToast({
        title: t("Join request approved", { defaultValue: "Join request approved" }),
        tone: "success",
      });
    },
    onError: (error) => {
      pushToast({
        title: t("Failed to approve join request", { defaultValue: "Failed to approve join request" }),
        body: error instanceof Error ? error.message : t("Unknown error", { defaultValue: "Unknown error" }),
        tone: "error",
      });
    },
  ...
  const rejectJoinRequestMutation = useMutation({
    ...
    onSuccess: async () => {
      await refreshAccessData();
      pushToast({
        title: t("Join request rejected", { defaultValue: "Join request rejected" }),
        tone: "success",
      });
    },
    onError: (error) => {
      pushToast({
        title: t("Failed to reject join request", { defaultValue: "Failed to reject join request" }),
        body: error instanceof Error ? error.message : t("Unknown error", { defaultValue: "Unknown error" }),
        tone: "error",
      });
    },
  ...
  if (!selectedCompanyId) {
    return <div className="text-sm text-muted-foreground">{t("Select a company to manage access.", {
      defaultValue: "Select a company to manage access.",
    })}</div>;
  }

  if (membersQuery.isLoading) {
    return <div className="text-sm text-muted-foreground">{t("Loading company access…", {
      defaultValue: "Loading company access…",
    })}</div>;
  }
    ...
    const message =
      membersQuery.error instanceof ApiError && membersQuery.error.status === 403
        ? t("You do not have permission to manage company members.", {
          defaultValue: "You do not have permission to manage company members.",
        })
        : membersQuery.error instanceof Error
          ? membersQuery.error.message
          : t("Failed to load company members.", { defaultValue: "Failed to load company members." });
    ...
    <div className="max-w-6xl space-y-8">
      <div className="space-y-3">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-lg font-semibold">{t("Company Access", { defaultValue: "Company Access" })}</h1>
        </div>
        <p className="max-w-3xl text-sm text-muted-foreground">
          {t("Manage company user memberships, membership status, and explicit permission grants for {{name}}.", {
            defaultValue: "Manage company user memberships, membership status, and explicit permission grants for {{name}}.",
      ...
      <section className="space-y-4">
        <div className="space-y-1">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-base font-semibold">{t("Humans", { defaultValue: "Humans" })}</h2>
          </div>
          <p className="max-w-3xl text-sm text-muted-foreground">
            {t("Manage human company memberships, status, and grants here.", {
              defaultValue: "Manage human company memberships, status, and grants here.",
            })}
          </p>
        </div>
        ...
        {access?.canApproveJoinRequests && pendingHumanJoinRequests.length > 0 ? (
          <div className="space-y-3 rounded-xl border border-border px-4 py-4">
            <div className="flex flex-wrap items-center justify-between gap-2">
              <div>
                <h3 className="text-sm font-semibold">{t("Pending human joins", { defaultValue: "Pending human joins" })}</h3>
                <p className="text-sm text-muted-foreground">
                  {t("Review human join requests before they become active company members.", {
                    defaultValue: "Review human join requests before they become active company members.",
                  })}
                </p>
              </div>
              <Badge variant="outline">
                {t("{{count}} pending", { defaultValue: "{{count}} pending", count: pendingHumanJoinRequests.length })}
              </Badge>
            </div>
            <div className="space-y-3">
              {pendingHumanJoinRequests.map((request) => (
                <PendingJoinRequestCard
                  ...
                  title={
                    ...
                    t("Unknown human requester", { defaultValue: "Unknown human requester" })
                  }
                  subtitle={
                    request.requesterUser?.email ||
                    request.requestEmailSnapshot ||
                    request.requestingUserId ||
                    t("No email available", { defaultValue: "No email available" })
                  }
                  context={
                    request.invite
                  ...
                  detail={t("Submitted {{date}}", {
                    defaultValue: "Submitted {{date}}",
                    date: new Date(request.createdAt).toLocaleString(),
                  })}
                  approveLabel={t("Approve human", { defaultValue: "Approve human" })}
                  rejectLabel={t("Reject human", { defaultValue: "Reject human" })}
        ...
        <div className="overflow-hidden rounded-xl border border-border">
          ...
          {members.length === 0 ? (
            <div className="px-4 py-8 text-sm text-muted-foreground">
              {t("No user memberships found for this company yet.", {
                defaultValue: "No user memberships found for this company yet.",
              })}
            </div>
          ) : (
            members.map((member) => (
              ...
              >
                ...
                <div className="min-w-0 text-sm text-muted-foreground">
                  {member.grants.length === 0
                    ? t("No explicit grants", { defaultValue: "No explicit grants" })
                    : member.grants.map((grant) => t(permissionLabels[grant.permissionKey], {
                      defaultValue: permissionLabels[grant.permissionKey],
                    })).join(", ")}
                </div>
      ...
      <Dialog open={!!editingMember} onOpenChange={(open) => !open && setEditingMemberId(null)}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            <DialogTitle>{t("Edit member", { defaultValue: "Edit member" })}</DialogTitle>
            <DialogDescription>
              {t("Update company role, membership status, and explicit grants for {{name}}.", {
                defaultValue: "Update company role, membership status, and explicit grants for {{name}}.",
            ...
            <div className="space-y-5">
              <div className="grid gap-4 md:grid-cols-2">
                <label className="space-y-2 text-sm">
                  <span className="font-medium">{t("Company role", { defaultValue: "Company role" })}</span>
                  <select
                ...
                <label className="space-y-2 text-sm">
                  <span className="font-medium">{t("Membership status", { defaultValue: "Membership status" })}</span>
                  <select
                    className="w-full rounded-md border border-border bg-background px-3 py-2"
                    value={draftStatus}
                    onChange={(event) =>
              ...
              <div className="space-y-3">
                <div>
                  <h3 className="text-sm font-medium">{t("Grants", { defaultValue: "Grants" })}</h3>
                  <p className="text-sm text-muted-foreground">
                ...
                <div className="rounded-lg border border-border px-3 py-3">
                  <div className="text-sm font-medium">{t("Implicit grants from role", { defaultValue: "Implicit grants from role" })}</div>
                    ...
                    <div className="mt-3 flex flex-wrap gap-2">
                      {implicitGrantKeys.map((permissionKey) => (
                        <Badge key={permissionKey} variant="outline">
                          {t(permissionLabels[permissionKey], { defaultValue: permissionLabels[permissionKey] })}
                        </Badge>
                      ))}
                    </div>
                  ) : null}
                        ...
                        {draftGrants.has(permissionKey) ? (
                          <span className="block text-xs text-muted-foreground">
                            {t("Stored explicitly for this member.", {
                              defaultValue: "Stored explicitly for this member.",
                            })}
                          </span>
                        ) : null}
                      </span>
          ...
          <DialogFooter>
            ...
            >
              {updateMemberMutation.isPending
                ...
                : t("Save access", { defaultValue: "Save access" })}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
