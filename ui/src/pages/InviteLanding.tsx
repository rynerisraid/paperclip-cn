...
function mapInviteAuthFeedback(
  ...
  const emailLabel = email.trim().length > 0
    ? email.trim()
    : t("inviteLanding.thatEmail", { defaultValue: "that email" });

  if (code === "USER_ALREADY_EXISTS_USE_ANOTHER_EMAIL") {
    return {
      tone: "info",
      message: t("inviteLanding.accountExistsForEmail", {
  ...
  if (code === "INVALID_EMAIL_OR_PASSWORD") {
    return {
      tone: "error",
      message: t("inviteLanding.invalidCredentials", {
        defaultValue:
  ...
  return {
    tone: "error",
    message: message ?? t("inviteLanding.authFailed", { defaultValue: "Authentication failed" }),
  };
}
...
function AwaitingJoinApprovalPanel({
  ...
  const approvalPageLabel = t("inviteLanding.companySettingsAccess", {
    defaultValue: "Company Settings \u2192 Access",
  });
      ...
      <div className="mx-auto max-w-md border border-zinc-800 bg-zinc-950 p-6" data-testid="invite-pending-approval">
        ...
        <div className="mt-4 space-y-3">
          <p className="text-sm text-zinc-400">
            {t("inviteLanding.awaitingApproval", {
          ...
          <div className="border border-zinc-800 p-3">
            <p className="text-xs text-zinc-500 mb-1">
              {t("inviteLanding.approvalPage", { defaultValue: "Approval page" })}
            </p>
            <a
              href={approvalUrl}
          ...
          <p className="text-sm text-zinc-400">
            {t("inviteLanding.askApproverVisitPrefix", {
              defaultValue: "Ask them to visit ",
            })}
            <a href={approvalUrl} className="text-zinc-200 underline underline-offset-2 hover:text-zinc-100">
              {approvalPageLabel}
            </a>
            {t("inviteLanding.askApproverVisitSuffix", {
        ...
        {claimSecret && claimApiKeyPath ? (
          <div className="mt-4 space-y-1 border border-zinc-800 p-3 text-xs text-zinc-400">
            <div className="text-zinc-200">
              {t("inviteLanding.claimSecret", { defaultValue: "Claim secret" })}
            </div>
        ...
        {onboardingTextUrl ? (
          <div className="mt-4 text-xs text-zinc-400">
            {t("inviteLanding.onboarding", { defaultValue: "Onboarding" })}:{" "}
...
export function InviteLandingPage() {
  ...
  const companyDisplayName = companyName || t("inviteLanding.thisCompany", {
    defaultValue: "this Paperclip company",
  });
  ...
  const requestedHumanRole = invite?.humanRole
    ? t(formatHumanRole(invite.humanRole) ?? invite.humanRole, {
      defaultValue: formatHumanRole(invite.humanRole) ?? invite.humanRole,
    })
    : null;
  ...
  const acceptMutation = useMutation({
    mutationFn: async () => {
      ...
      if (isCheckingExistingMembership) {
        throw new Error(t("inviteLanding.checkingAccess", {
          defaultValue: "Checking your company access. Try again in a moment.",
        }));
      }
      if (isCurrentMember) {
        throw new Error(t("inviteLanding.accountAlreadyBelongs", {
          defaultValue: "This account already belongs to the company.",
        }));
      }
    ...
    onError: (err) => {
      setError(err instanceof Error ? err.message : t("inviteLanding.failedAcceptInvite", {
  ...
  const joinButtonLabel = useMemo(() => {
    if (!invite) return t("inviteLanding.continue", { defaultValue: "Continue" });
    if (invite.inviteType === "bootstrap_ceo") {
      return t("inviteLanding.acceptInvite", { defaultValue: "Accept invite" });
    }
    ...
    return sessionQuery.data
      ...
      : t("inviteLanding.continue", { defaultValue: "Continue" });
  }, [invite, sessionQuery.data, showsAgentForm, t]);

  if (!token) {
    return (
      <div className="mx-auto max-w-xl py-10 text-sm text-destructive">
        {t("inviteLanding.invalidToken", { defaultValue: "Invalid invite token." })}
      </div>
    );
  }
  ...
  if (inviteQuery.isLoading || healthQuery.isLoading || sessionQuery.isLoading) {
    return (
      <div className="mx-auto max-w-xl py-10 text-sm text-muted-foreground">
        {t("inviteLanding.loadingInvite", { defaultValue: "Loading invite..." })}
      </div>
    );
  }
  ...
  if (isCheckingExistingMembership) {
    return (
      <div className="mx-auto max-w-xl py-10 text-sm text-muted-foreground">
        {t("inviteLanding.checkingYourAccess", { defaultValue: "Checking your access..." })}
      </div>
    );
  }
    ...
    return (
      <div className="mx-auto max-w-xl py-10">
        <div className="border border-border bg-card p-6" data-testid="invite-error">
          <h1 className="text-lg font-semibold">
            {t("inviteLanding.inviteUnavailable", { defaultValue: "Invite not available" })}
          </h1>
          <p className="mt-2 text-sm text-muted-foreground">
            {t("inviteLanding.inviteUnavailableBody", {
  ...
  if (
    ...
    return (
      <div className="mx-auto max-w-xl py-10 text-sm text-muted-foreground">
        {t("inviteLanding.openingCompany", { defaultValue: "Opening company..." })}
      </div>
    );
  }
        ...
        <div className="border border-border bg-card p-6" data-testid="invite-error">
          ...
          <p className="mt-2 text-sm text-muted-foreground">
            {inviteJoinRequestStatus === "rejected"
              ? t("inviteLanding.joinRequestRejected", {
    ...
    return (
      <div className="min-h-screen bg-zinc-950 px-6 py-12 text-zinc-100">
        <div className="mx-auto max-w-md border border-zinc-800 bg-zinc-950 p-6">
          <h1 className="text-lg font-semibold">
            {t("inviteLanding.bootstrapComplete", { defaultValue: "Bootstrap complete" })}
          </h1>
          <div className="mt-4">
        ...
        <div className="min-h-screen bg-zinc-950 px-6 py-12 text-zinc-100">
          <div className="mx-auto max-w-md border border-zinc-800 bg-zinc-950 p-6">
            <div className="flex items-center gap-3">
              ...
              />
              <h1 className="text-lg font-semibold">
                {t("inviteLanding.joinedCompany", { defaultValue: "You joined the company" })}
              </h1>
            </div>
            <div className="mt-4">
        ...
        <div className="grid gap-6 lg:grid-cols-[minmax(0,1.15fr)_minmax(360px,0.85fr)]">
          <section className={`${panelClassName} space-y-6`}>
            <div className="flex items-start gap-4">
              ...
              <div className="min-w-0">
                ...
                <p className="mt-2 max-w-2xl text-sm leading-6 text-zinc-300">
                  {showsAgentForm
                    ? t("inviteLanding.agentReviewIntro", {
                    ...
                    : requiresHumanAccount
                      ? t("inviteLanding.createAccountIntro", {
                      ...
                      : t("inviteLanding.accountReadyIntro", {
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              ...
              <div className="border border-zinc-800 p-3">
                <div className="text-xs uppercase tracking-[0.2em] text-zinc-500">
                  {t("Invited by", { defaultValue: "Invited by" })}
                </div>
                <div className="mt-1 text-sm text-zinc-100">
                  {invitedByUserName ?? t("inviteLanding.paperclipBoard", { defaultValue: "Paperclip board" })}
                </div>
              </div>
              <div className="border border-zinc-800 p-3">
                <div className="text-xs uppercase tracking-[0.2em] text-zinc-500">
                  {t("inviteLanding.requestedAccess", { defaultValue: "Requested access" })}
                </div>
                <div className="mt-1 text-sm text-zinc-100">
                  {showsAgentForm
                    ? t("inviteLanding.agentJoinRequest", { defaultValue: "Agent join request" })
                    : requestedHumanRole ?? t("inviteLanding.companyAccessLabel", { defaultValue: "Company access" })}
                </div>
              </div>
              <div className="border border-zinc-800 p-3">
                <div className="text-xs uppercase tracking-[0.2em] text-zinc-500">
                  {t("inviteLanding.inviteExpires", { defaultValue: "Invite expires" })}
                </div>
            ...
            {inviteMessage ? (
              <div className="border border-amber-500/40 bg-amber-500/10 p-4">
                <div className="text-xs uppercase tracking-[0.2em] text-amber-200/80">
                  {t("inviteLanding.messageFromInviter", { defaultValue: "Message from inviter" })}
                </div>
            ...
            {sessionQuery.data ? (
              <div className="border border-emerald-500/40 bg-emerald-500/10 p-4 text-sm text-emerald-50">
                {t("inviteLanding.signedInAs", {
                  defaultValue: "Signed in as {{account}}.",
                  account: sessionLabel,
                })}
              </div>
            ) : null}
          </section>
            ...
            {showsAgentForm ? (
              <div className="space-y-4">
                <div>
                  <h2 className="text-lg font-semibold">
                    {t("inviteLanding.submitAgentDetails", { defaultValue: "Submit agent details" })}
                  </h2>
                  <p className="mt-1 text-sm text-zinc-400">
                    {t("inviteLanding.agentApprovalRequest", {
                ...
                <label className="block text-sm">
                  <span className="mb-1 block text-zinc-400">
                    {t("Agent name", { defaultValue: "Agent name" })}
                  </span>
                  <input
                ...
                <label className="block text-sm">
                  <span className="mb-1 block text-zinc-400">
                    {t("Adapter type", { defaultValue: "Adapter type" })}
                  </span>
                  <select
                    className={fieldClassName}
                    value={adapterType}
              ...
              <div className="space-y-5">
                ...
                <div className="flex gap-2">
                  ...
                  >
                    {t("inviteLanding.haveAccount", { defaultValue: "I already have an account" })}
                  </button>
                </div>
                ...
                <form
                  ...
                  onSubmit={(event) => {
                    ...
                    if (!authCanSubmit) {
                      setAuthFeedback({
                        tone: "error",
                        message: t("inviteLanding.fillRequired", {
                ...
                <div>
                  <h2 className="text-lg font-semibold">
                    {shouldAutoAcceptHumanInvite
                      ? t("inviteLanding.submittingJoinRequest", { defaultValue: "Submitting join request" })
                      : invite.inviteType === "bootstrap_ceo"
                  ...
                  <p className="mt-1 text-sm text-zinc-400">
                    {shouldAutoAcceptHumanInvite
                      ? t("inviteLanding.submittingForCompany", {
                        defaultValue: "Submitting your join request for {{company}}.",
                        company: companyDisplayName,
                      })
                      : isCurrentMember
                      ? t("inviteLanding.alreadyBelongsToCompany", {
                        defaultValue: "This account already belongs to {{company}}.",
                        company: companyDisplayName,
                      })
                      : invite.inviteType === "bootstrap_ceo"
                        ? t("inviteLanding.thisWillSetUpPaperclip", {
                          defaultValue: "This will finish setting up Paperclip.",
                        })
                        : t("inviteLanding.thisWillSubmitJoinRequest", {
                          defaultValue: "This will submit or complete your join request for {{company}}.",
                          company: companyDisplayName,
                        })}
                  </p>
                </div>
