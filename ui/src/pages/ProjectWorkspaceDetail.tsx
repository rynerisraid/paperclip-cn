  ...
  const controlRuntimeServices = useMutation({
    ...
    onSuccess: (result, action) => {
      ...
      setRuntimeActionMessage(
        action === "stop"
          ? t("projectWorkspace.runtimeServicesStopped")
          : action === "restart"
            ? t("projectWorkspace.runtimeServicesRestarted")
            : t("projectWorkspace.runtimeServicesStarted"),
      );
    },
    onError: (error) => {
      setRuntimeActionMessage(null);
      setErrorMessage(error instanceof Error ? error.message : t("projectWorkspace.failedToControlRuntime"));
    },
  });
        ...
        <div className="space-y-6">
          <div className="rounded-2xl border border-border bg-card p-5">
            ...
            <div className="mt-4 grid gap-4">
              ...
              <Field label={t("projectWorkspace.runtimeServicesJson")} hint={t("projectWorkspace.runtimeServicesJsonHint")}>
                <textarea
        ...
        <div className="space-y-6">
          ...
          <div className="rounded-2xl border border-border bg-card p-5">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
              <div className="space-y-1">
                <div className="text-xs font-medium uppercase tracking-[0.16em] text-muted-foreground">{t("projectWorkspace.runtimeServices")}</div>
                <h2 className="text-lg font-semibold">{t("projectWorkspace.attachedServices")}</h2>
                <p className="text-sm text-muted-foreground">
                  {t("projectWorkspace.attachedServicesDesc")}
                </p>
              </div>
            ...
            {workspace.runtimeServices && workspace.runtimeServices.length > 0 ? (
                    ...
                    <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                      <div className="space-y-1">
                        ...
                        <div className="space-y-1 text-xs text-muted-foreground">
                          ...
                          {service.port ? <div>{t("projectWorkspace.port", { port: service.port })}</div> : null}
                          <div>{service.command ?? t("projectWorkspace.noCommandRecorded")}</div>
              ...
              <p className="text-sm text-muted-foreground">
                {workspace.runtimeConfig?.workspaceRuntime
                  ? t("projectWorkspace.noServicesRunning")
                  : t("projectWorkspace.noServicesConfigured")}
              </p>
            )}
          </div>
        </div>
