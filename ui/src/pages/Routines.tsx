    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div className="space-y-1">
          <h1 className="text-2xl font-semibold tracking-tight flex items-center gap-2">
            ...
            <span className="rounded-full bg-amber-100 px-2 py-0.5 text-xs font-medium text-amber-800 dark:bg-amber-900/30 dark:text-amber-400">{t("Beta", { defaultValue: "Beta" })}</span>
          </h1>
      ...
      >
        <DialogContent showCloseButton={false} className="max-w-3xl gap-0 overflow-hidden p-0">
          <div className="flex flex-wrap items-center justify-between gap-3 border-b border-border/60 px-5 py-3">
            <div>
              <p className="text-xs font-medium uppercase tracking-[0.2em] text-muted-foreground">{t("New routine", { defaultValue: "New routine" })}</p>
              <p className="text-sm text-muted-foreground">
                {t("Define the recurring work first. Trigger setup comes next on the detail page.", {
                  defaultValue: "Define the recurring work first. Trigger setup comes next on the detail page.",
                })}
              </p>
            </div>
            <Button
          ...
          <div className="border-t border-border/60 px-5 py-3">
            <Collapsible open={advancedOpen} onOpenChange={setAdvancedOpen}>
              <CollapsibleTrigger className="flex w-full items-center justify-between text-left">
                <div>
                  <p className="text-sm font-medium">{t("Advanced delivery settings", { defaultValue: "Advanced delivery settings" })}</p>
                  <p className="text-sm text-muted-foreground">{t("Keep policy controls secondary to the work definition.", { defaultValue: "Keep policy controls secondary to the work definition." })}</p>
                </div>
                    ...
                    >
                      ...
                      <SelectContent>
                        {concurrencyPolicies.map((value) => (
                          <SelectItem key={value} value={value}>{t(value.replaceAll("_", " "), { defaultValue: value.replaceAll("_", " ") })}</SelectItem>
                        ))}
                      </SelectContent>
                    </Select>
                    <p className="text-xs text-muted-foreground">{t(concurrencyPolicyDescriptions[draft.concurrencyPolicy], { defaultValue: concurrencyPolicyDescriptions[draft.concurrencyPolicy] })}</p>
                  </div>
                  <div className="space-y-2">
                    ...
                    <p className="text-xs text-muted-foreground">{t(catchUpPolicyDescriptions[draft.catchUpPolicy], { defaultValue: catchUpPolicyDescriptions[draft.catchUpPolicy] })}</p>
                  </div>
                </div>
              </CollapsibleContent>
            </Collapsible>
      ...
      {error ? (
          ...
          <CardContent className="pt-6 text-sm text-destructive">
            {error instanceof Error ? error.message : t("Failed to load routines", { defaultValue: "Failed to load routines" })}
          </CardContent>
        </Card>
      ) : null}
            ...
            <table className="min-w-full text-sm">
              <thead>
                <tr className="text-left text-xs text-muted-foreground border-b border-border">
                  <th className="px-3 py-2 font-medium">{t("Name", { defaultValue: "Name" })}</th>
                  <th className="px-3 py-2 font-medium">{t("Project", { defaultValue: "Project" })}</th>
                  <th className="px-3 py-2 font-medium">{t("Agent", { defaultValue: "Agent" })}</th>
                  <th className="px-3 py-2 font-medium">{t("Last run", { defaultValue: "Last run" })}</th>
                  <th className="px-3 py-2 font-medium">{t("Enabled", { defaultValue: "Enabled" })}</th>
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-3 py-2.5">
                        {routine.projectId ? (
                          <div className="flex items-center gap-2 text-sm text-muted-foreground">
                            ...
                            />
                            <span className="truncate">{projectById.get(routine.projectId)?.name ?? t("Unknown", { defaultValue: "Unknown" })}</span>
                          </div>
                        ) : (
                      ...
                      <td className="px-3 py-2.5">
                        {routine.assigneeAgentId ? (() => {
                          ...
                          return agent ? (
                            ...
                            <span className="text-xs text-muted-foreground">{t("Unknown", { defaultValue: "Unknown" })}</span>
                          );
                        })() : (
                          <span className="text-xs text-muted-foreground">—</span>
                        )}
                      ...
                      <td className="px-3 py-2.5 text-muted-foreground">
                        ...
                        {routine.lastRun ? (
                          <div className="mt-1 text-xs">{t(routine.lastRun.status.replaceAll("_", " "), { defaultValue: routine.lastRun.status.replaceAll("_", " ") })}</div>
                        ) : null}
                      </td>
                      <td className="px-3 py-2.5" onClick={(e) => e.stopPropagation()}>
                        <div className="flex items-center gap-3">
                          <button
                            ...
                            aria-label={enabled
                              ? t("Disable {{name}}", { name: routine.title, defaultValue: `Disable ${routine.title}` })
                              : t("Enable {{name}}", { name: routine.title, defaultValue: `Enable ${routine.title}` })}
                      ...
                      <td className="px-3 py-2.5 text-right" onClick={(e) => e.stopPropagation()}>
                        <DropdownMenu>
                          <DropdownMenuTrigger asChild>
                            <Button variant="ghost" size="icon-sm" aria-label={t("More actions for {{name}}", { name: routine.title, defaultValue: `More actions for ${routine.title}` })}>
