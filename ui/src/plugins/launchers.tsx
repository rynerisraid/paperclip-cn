...
function launcherShellBoundsStyle(bounds: PluginLauncherBounds | null): CSSProperties {
  switch (bounds) {
    ...
    case "full":
      return { width: "calc(100vw - 2rem)", height: "calc(var(--paperclip-available-height) - 2rem)" };
    case "inline":
...
function LauncherModalShell({
  ...
  const panelClassName = shellType === "openDrawer"
    ? "fixed right-0 top-0 h-[var(--paperclip-available-height)] max-w-[min(44rem,100vw)] overflow-hidden border-l border-border bg-background shadow-2xl"
    : shellType === "openPopover"
      ? "fixed overflow-hidden rounded-xl border border-border bg-background shadow-2xl"
      : "fixed left-1/2 top-1/2 max-h-[calc(var(--paperclip-available-height)-2rem)] -translate-x-1/2 -translate-y-1/2 overflow-hidden rounded-2xl border border-border bg-background shadow-2xl";
    ...
    <>
      ...
      >
        ...
        <div
          className={cn(
            "overflow-auto p-4",
            shellType === "openDrawer" ? "h-[calc(100%-3.5rem)]" : "max-h-[calc(var(--paperclip-available-height)-7rem)]",
          )}
        >
